...
<?php 
	$this->title="Admin - nouvel article";
		// Si création article impossible message erreur
...
<form action="../public/index.php?ad&action=create" method="post" onsubmit="return checkFormNewPost()">
	...
	<label>
		<h2>Titre</h2>
		<input type="text" name="title" id="myTitle" class="form-control" id="usr" required maxlength="20">
	</label> <br/>

	<label id="post">
		<h2>Votre article</h2>
		<textarea name="content" id="myPost" class="mceEditor" ></textarea>
	</label> <br/>

	<label id="Extract">
		<h2>Votre extrait</h2>
		<textarea name="extract" class= "form-control" id="myExtract" required maxlength="255" onkeyup="javascript:checkTextArea();" ></textarea>
	</label><br/><br/><br/><br/>
