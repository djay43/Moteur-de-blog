	...
	?>		  
			...
			<span id="delete_view">
				<input type="checkbox" name="postId[]" value="<?= $post->getId();?>"> <h5><?= $post->getTitle();?></h5><br/>
					...
					<?= $post->getExtract();?> <br/>
					<a href="../public/index.php?ad&trash&action=delete_post&id=<?= $post-> getId();?>" onclick="return(confirm('Etes-vous sûr de vouloir supprimer cette entrée?'));"> <i class="fas fa-trash-alt" id="deleteIcon"></i></a>
			</span>
